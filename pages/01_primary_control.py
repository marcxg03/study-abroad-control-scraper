...
def render_downloads() -> None:
    """Render download button for the primary control CSV once exports are available."""
    ...
    st.download_button(
        label="⬇️ Download Primary Control CSV",
        data=file_bytes,
        file_name="primary_control_studyAbroad.csv",
        mime="text/csv",
...
def main() -> None:
    """Render the Primary Control workflow page."""
    ...
    if id_status in ("idle", "done", "error"):
        if id_status == "error":
            st.error(id_progress.get("error") or "The app could not identify primary control users right now. Please try again.")
        if st.button("Identify Users", type="primary", use_container_width=True):
            ...
            def _run_identification(prog=id_prog, kw=keywords, n=int(sample_size)):
                def callback(posts_scanned, users_found):
                    prog["posts_scanned"] = posts_scanned
                    prog["users_found"] = users_found
                try:
                    result = identify_primary_control(keywords=kw, target_n=n, progress_callback=callback)
                    prog["result"] = result
                    prog["status"] = "done"
