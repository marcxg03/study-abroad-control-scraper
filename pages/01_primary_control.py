...
from config import CANCELLATION_KEYWORDS, DEFAULT_SAMPLE_SIZE, GROUPS
...
def main() -> None:
    ...
    if st.button("Identify Users", type="primary", use_container_width=True):
        ...
        try:
            st.session_state.identified_users = identify_primary_control(
                keywords=keywords,
                target_n=int(sample_size),
            )
        except Exception:
            st.error("The app could not identify primary control users right now. Please try again.")
